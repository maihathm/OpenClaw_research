...
description: >
  ...
  browser-use tools, coding agents, or OpenClaw-related tooling. Also use when
  a source requires browser automation fallback after normal search/fetch is
  insufficient.
---
...
1. Read `profiles/openclaw-agents.json` or another domain profile.
2. Read `references/CRAWL_PLAN.md` for source priorities and cadence.
3. Use `references/AGENT_PROMPT.md` when you need a reusable prompt shell.
4. Save every run to `research/<profile-or-domain>/YYYY-MM-DD-<slug>.md`.
...
- output target: note / digest / database update / experiment proposal
...
- documentation sites that require UI navigation to reveal versioned content
...
The default profile in this repo is `openclaw-agents`, focused on:

- OpenClaw ecosystem
- agent tooling
- browser-use automation
- evals, safety, and operational lessons
- practical engineering posts and field tips
